...
cycle = function(){
  ...
  for(i=0; network.nodeList[i] != null; i++){
    ...
    isOverCircle = mo.fCircleM(node.x+mo.cX, node.y+mo.cY, 2*node.nodeList.length+2);
    if(isOverCircle) {
      iOver = i;
    }
  }
  ...
  if(iOver != null){
    mo.setStroke('black', 4);
    node = network.nodeList[iOver];
    mo.sCircle(node.x+mo.cX, node.y+mo.cY, 2*node.nodeList.length + 10);
